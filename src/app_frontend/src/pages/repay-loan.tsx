...
export function RepayLoan() {
  ...
  const [collateralCkBTC, setCollateralCkBTC] = useState(0);
  const [usdcBalance, setUsdcBalance] = useState(0);
  const [btcUsd, setBtcUsd] = useState<number | null>(null);
  const [borrowDate, setBorrowDate] = useState<string | null>(null);
  const [daysActive, setDaysActive] = useState<string | null>(null);
  const interestUSD = 0;
  const totalDueUSD = debtUSDC + interestUSD;
  const collateralReleaseCkBTC = (collateralCkBTC * percentage) / 100;
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (pos) {
          setDebtUSDC(Number(pos.debt_ckusdc || 0) / 1e6);
          setCollateralCkBTC(Number(pos.collateral_ckbtc || 0) / 1e8);
        }
        const bal = await (ckusdc as any).icrc1_balance_of({
          owner: identity.getPrincipal(),
          subaccount: [],
        });
        setUsdcBalance(Number(bal || 0) / 1e6);

        // Load prices
        try {
          const prices = await (mainCanister as any).getPrices();
          if (prices && typeof prices.btc_usd_e8s !== "undefined") {
            setBtcUsd(Number(prices.btc_usd_e8s) / 1e8);
          }
        } catch (e) {
          console.warn("Failed to load prices", e);
        }

        // Load last borrow activity to derive borrow date and days active
        try {
          const acts = await (mainCanister as any).getActivity(
            identity.getPrincipal()
          );
          if (Array.isArray(acts) && acts.length > 0) {
            const borrowActs = acts.filter(
              (a: any) => a && a.kind === "borrow"
            );
            if (borrowActs.length > 0) {
              const latest = borrowActs[0];
              const ms = Number(latest.time) / 1_000_000; // Time.now() ns → ms
              const d = new Date(ms);
              setBorrowDate(d.toLocaleDateString());
              const days = Math.max(
                0,
                Math.floor((Date.now() - ms) / (1000 * 60 * 60 * 24))
              );
              setDaysActive(String(days));
            }
          }
        } catch (e) {
        ...
        console.error("Failed to load position/balance", e);
      }
    };
    load();
  }, [identity, mainCanister, ckusdc]);
    ...
    try {
      ...
      if ("Err" in approveRes) {
        toast({
          ...
          description: JSON.stringify(approveRes.Err),
        });
        setLoading(false);
        return;
      }
      ...
      if ("Err" in repayRes) {
        ...
        toast({
          title: "Repay succeeded",
          description: `Block index: ${String(repayRes.Ok)}`,
        });
  ...
  const repayAmount = Math.min(debtUSDC, (debtUSDC * percentage) / 100).toFixed(
    2
  );
              ...
              <div className="bg-bg-tertiary rounded-2xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAmount(sanitizeDecimalInput(e.target.value, 6))
                    }
                    placeholder="0.00"
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex justify-between items-center py-3">
                ...
                <span className="body-regular font-medium text-semantic-positive">
                  {collateralReleaseCkBTC.toFixed(8)} ckBTC
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="body-small text-text-secondary">
                  Partial repayments are allowed. Your collateral will be
                  released proportionally to the amount repaid. The remaining
                  loan will continue to accrue interest.
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="card-container">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="body-small font-medium text-accent-teal">
                  —
                </span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                <span className="body-small text-text-secondary">LTV</span>
                <span className="body-small font-medium text-semantic-positive">
                  {(() => {
                    const collateralUsd = btcUsd
                      ? collateralCkBTC * btcUsd
                      : null;
                    if (!collateralUsd || collateralUsd <= 0) return "—";
                    const pct = Math.min(
                      100,
                      Math.max(0, (debtUSDC / collateralUsd) * 100)
                    );
      ...
      {showConfirmation && (
        ...
        >
          ...
          >
            ...
            <h3 className="heading-medium text-text-primary text-center mb-2">
              Repayment Successful!
            </h3>
            <p className="body-regular text-text-secondary text-center mb-6">
              You've repaid ${repayAmount} USDC. Your collateral of{" "}
              {collateralReleaseCkBTC.toFixed(8)} ckBTC has been released to
              your wallet.
            </p>
            <button
